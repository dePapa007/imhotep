...
import { buildUsersCsv, csvFilename } from "@/server/reports/exports";
...
export async function GET(request: Request) {
  ...
  const csv = await buildUsersCsv(range);
  return new NextResponse(csv, {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": `attachment; filename="${csvFilename("users", parsed)}"`,
    },
  });
}
